...
class SaveUserActivity():AppCompatActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        id = intent.getLongExtra(MainActivity.UpdateUserData, -1)

        if(id > 0){
            val data = mDbHandler.getUserWithId(id)
            setUserData(data)
            btn_add.text = "수 정"
        }
    }
    ...
    private fun getPicture(path:String): Drawable?
    {
        ...
        var bitmap:Bitmap? = MediaStore.Images.Thumbnails.getThumbnail(contentResolver,img_id,
    ...
    fun setUserData(userData:UserInfo){
        sel_image.image = getPicture(userData.pic_path)
        edit_name.text = Editable.Factory.getInstance().newEditable(userData.name)
        edit_age.text = Editable.Factory.getInstance().newEditable(userData.age)
        edit_tel.text = Editable.Factory.getInstance().newEditable(userData.TelNum)
    }
    ...
    {
        val user:UserInfo = UserInfo(id, edit_name.text.toString(), edit_age.text.toString(),
